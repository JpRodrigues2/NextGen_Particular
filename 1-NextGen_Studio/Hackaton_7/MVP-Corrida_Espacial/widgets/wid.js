...
angular.module("espacial", []).controller("espacial", [
  ...
  ($scope, $http, $timeout, $interval) => {
    ...
    // Melhorar a função para tocar o som de sino quando o primeiro lugar mudar
    function initAudio() {
      try {
    ...
    function processLeaderboardData(data) {
      ...
      // Substituir a verificação de mudança de primeiro lugar na função processLeaderboardData
    ...
    // Get ship image based on position
    $scope.getShipImage = (position) => {
      // Special ships for top 3 positions
      if (position === 1) return $scope.getAssetUrl("nave-ouro.png")
      if (position === 2) return $scope.getAssetUrl("nave-prata.png")
      if (position === 3) return $scope.getAssetUrl("nave-bronze.png")

      // For other positions, use color-based ships
      const colors = ["red", "blue", "green", "yellow"]
      const colorIndex = (position - 1) % colors.length
      return $scope.getAssetUrl("nave" + (colorIndex + 1) + ".png")
    }
