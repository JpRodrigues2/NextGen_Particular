angular.module("RankingApp", []).controller("RankingController", ($scope, $http, $timeout, $window, $interval) => {
  ...
  $scope.showPlayerSidebar = (player) => {
    ...
    document.getElementById("sidebar-points").textContent = player.total + " pts"

    // Definir o tipo de carro com base na posição
    let carType = ""
    if (player.position === 1) carType = "Ouro"
    else if (player.position === 2) carType = "Prata"
    else if (player.position === 3) carType = "Bronze"
    else carType = "Regular"
    document.getElementById("sidebar-car").textContent = carType
