$scope.obj = {
  ...
  font: "Roboto",
  text_color: "#2d5c5c",
  update_interval: 30000, // 30 segundos padrão
  visible_types: {
    challenges: true,
    actions: true,
  },
...
$scope.save = () => {
  var req = {
    method: "PUT",
    url: Marketplace.auth.getService() + "/v3/database/notificacoes__c",
    headers: {
...
$scope.load = () => {
  $http({
    method: "GET",
    url: Marketplace.auth.getService() + "/v3/database/notificacoes__c?strict=true&q=_id:'global'",
    headers: {
      Authorization: Marketplace.auth.getAuthorization(),
      "content-type": "application/json",
      "cache-control": "no-cache",
...
$scope.loadFont = (fontName) => {
  ...
  var linkId = "dynamic-font-link-notificacoes-admin";
...
// Apply Banco do Brasil style
$scope.applyBancoDoBrasilStyle = () => {
  $scope.obj.background_color = "#ffffff"; // White background
  $scope.obj.header_color = "#0066b3"; // Banco do Brasil blue
  $scope.obj.border_color = "#ffcd00"; // Yellow border
  $scope.obj.font = "Roboto"; // Keep Roboto for consistency
  $scope.obj.text_color = "#ffffff"; // White text for contrast on dark backgrounds
  $scope.obj.update_interval = 30000; // Maintain default
  $scope.obj.visible_types = {
    challenges: true,
    actions: true,
  };
  $scope.save(); // Save the new configuration
};
...
$scope.resetToDefault = () => {
  ...
  $http({
    method: "DELETE",
    url: Marketplace.auth.getService() + "/v3/database/notificacoes__c?q=*",
    headers: {
      Authorization: Marketplace.auth.getAuthorization(),
      "content-type": "application/json",
      "cache-control": "no-cache",
